...
const Terminal: React.FC<TerminalProps> = ({ onClose }) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    ...
    { role: 'model', text: 'Systems online. Awaiting driver input. Enter API KEY to engage conversational matrix.' }
  ]);
  ...
  const handleKeySubmit = (e: React.FormEvent) => {
    ...
    if(apiKey.trim().length > 10) {
        setHasKey(true);
        setMessages(prev => [...prev, { role: 'system', text: 'API KEY ACCEPTED. NEURAL LINK ESTABLISHED.' }]);
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: text || "ERROR: SIGNAL INTERRUPTED" }]);
    } catch (error) {
      setMessages(prev => [...prev, { role: 'system', text: 'ERROR: CONNECTION FAILED. CHECK CREDENTIALS.' }]);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-2xl bg-[#050505] border border-[#333] shadow-[0_0_50px_rgba(255,0,60,0.2)] flex flex-col h-[600px] font-mono text-sm relative overflow-hidden">
        ...
        <div className="flex-1 p-4 overflow-y-auto scrollbar-thin scrollbar-thumb-[#333] text-[#00F0FF]">
          {messages.map((msg, idx) => (
            <div key={idx} className={`mb-3 ${msg.role === 'user' ? 'text-[#FCEE09] text-right' : msg.role === 'system' ? 'text-[#FF003C]' : 'text-[#00F0FF]'}`}>
              <span className="opacity-50 text-xs mr-2">[{msg.role === 'user' ? 'DRIVER' : msg.role === 'model' ? 'AI' : 'SYS'}]</span>
              {msg.text}
            </div>
          ))}
          {loading && <div className="text-[#FF003C] animate-pulse">PROCESSING_DATA...</div>}
        ...
        <div className="p-4 border-t border-[#333] bg-[#0a0a0a] relative z-20">
           {!hasKey ? (
               <form onSubmit={handleKeySubmit} className="flex gap-2">
                   ...
                   />
                   <button type="submit" className="text-[#FF003C] font-bold hover:text-white">AUTH</button>
               </form>
           ) : (
                <form onSubmit={handleSubmit} className="flex gap-2">
                    ...
                    <input 
                        ...
                        autoFocus
                    />
                    <button type="submit" disabled={loading} className="text-[#FCEE09] hover:text-white disabled:opacity-50">
                        <Send size={18} />
                    </button>
                </form>
           )}
